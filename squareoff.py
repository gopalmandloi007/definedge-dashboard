    ...
    with st.form(key=f"squareoff_hold_form_{tradingsymbols[0]['tradingsymbol']}"):
        ...
        if qty_option == "Partial":
            squareoff_qty = st.number_input(
                "Enter quantity to square off", min_value=1, max_value=int(qty), value=1
            )
        else:
        ...
        disclose = st.checkbox("Disclose Partial Quantity?")
        if disclose:
            disclosed_quantity = st.number_input(
                "Disclosed Quantity (optional)", min_value=1, max_value=int(squareoff_qty), value=1
            )
        else:
