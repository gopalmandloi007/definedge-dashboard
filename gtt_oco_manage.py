...
def show():
    ...
    if gttlist:
        for order in gttlist:
            with st.expander(
                f"GTT: {order.get('tradingsymbol', '')} ({order.get('alert_id', '')}) | "
                f"{order.get('order_type', '')} @ {order.get('alert_price', 0)} | Qty: {order.get('quantity', 1)}"
            ):
                st.json(order)
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("Modify", key=f"gtt_mod_{order.get('alert_id', '')}"):
                        with st.form(f"gtt_mod_form_{order.get('alert_id', '')}"):
                            condition = st.radio(
                                "Condition",
                                ["LTP_ABOVE", "LTP_BELOW"],
                                horizontal=True,
                            ...
                            if submit_mod:
                                ...
                                st.json(resp)
                with col2:
                    if st.button("Cancel", key=f"gtt_cancel_{order.get('alert_id', '')}"):
    ...
    if ocolist:
        for order in ocolist:
            with st.expander(
                f"OCO: {order.get('tradingsymbol', '')} ({order.get('alert_id', '')}) | {order.get('order_type', '')}"
            ):
                st.json(order)
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("Modify", key=f"oco_mod_{order.get('alert_id', '')}"):
                        with st.form(f"oco_mod_form_{order.get('alert_id', '')}"):
                            target_price = st.number_input(
                                "Target Price", value=float(order.get('target_price', 0)), key=f"oco_tg_{order.get('alert_id', '')}"
                            )
                            stoploss_price = st.number_input(
                                "Stoploss Price", value=float(order.get('stoploss_price', 0)), key=f"oco_sl_{order.get('alert_id', '')}"
                            )
                            target_qty = st.number_input(
                                "Target Quantity", value=int(order.get('target_quantity', 1)), step=1, key=f"oco_tq_{order.get('alert_id', '')}"
                            )
                            stoploss_qty = st.number_input(
                                "Stoploss Quantity", value=int(order.get('stoploss_quantity', 1)), step=1, key=f"oco_sq_{order.get('alert_id', '')}"
                            )
                            product_type = st.radio(
                                "Product Type",
                                ["CNC", "INTRADAY", "NORMAL"],
                            ...
                            remarks = st.text_input("Remarks", value=order.get('remarks', ''), key=f"oco_rem_{order.get('alert_id', '')}")
                            submit_mod = st.form_submit_button("Confirm Modify")
                            if submit_mod:
                                payload = {
                                    "remarks": remarks,
                                ...
                                st.json(resp)
                with col2:
                    if st.button("Cancel", key=f"oco_cancel_{order.get('alert_id', '')}"):
                        api_session_key = st.secrets.get("integrate_api_session_key", "")
